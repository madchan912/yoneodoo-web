...
export default function IngredientNormalizePage() {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#888', fontSize: '0.9rem', maxWidth: 720 }}>
        레시피 JSON에만 있는 원본 재료명 중, 아직 <code style={{ color: '#93c5fd' }}>ingredient_mapping</code>에 없는 항목만
        표시됩니다. (이름은 서버에서 공백 제거 규칙으로 정규화됩니다.)
      </p>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontWeight: 'bold', color: '#e5e5e5' }}>미분류 원본 재료</span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                type="button"
                onClick={selectAll}
                style={{ fontSize: '0.75rem', padding: '4px 8px', borderRadius: 6, border: '1px solid #555', background: '#2d2d2d', color: '#ddd', cursor: 'pointer' }}
              >
                전체 선택
              </button>
              <button
          ...
          {loading ? (
            ...
            <div style={{ color: '#666', textAlign: 'center', padding: 24 }}>미분류 재료가 없습니다. (전부 매핑됨 또는 데이터 없음)</div>
          ) : (
        ...
        >
          <div style={{ fontWeight: 'bold', color: '#e5e5e5', marginBottom: 12 }}>마스터 재료명으로 묶기</div>
